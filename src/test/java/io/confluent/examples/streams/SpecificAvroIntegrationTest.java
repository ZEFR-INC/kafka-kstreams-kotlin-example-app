...
public class SpecificAvroIntegrationTest {
  ...
  public void shouldRoundTripSpecificAvroDataThroughKafka() throws Exception {
    List<VideoMessage> inputValues = Collections.singletonList(
        VideoMessage.newBuilder().setPayload(Video.newBuilder().setTitle("My Awesome video").build()).build()
    );
    ...
    final Serde<VideoMessage> specificAvroSerde = new SpecificAvroSerde<>();
    ...
    specificAvroSerde.configure(
        ...
        isKeySerde);
    KStream<String, VideoMessage> stream = builder.stream(inputTopic);
    ...
    List<VideoMessage> actualValues = IntegrationTestUtils.waitUntilMinValuesRecordsReceived(consumerConfig,
